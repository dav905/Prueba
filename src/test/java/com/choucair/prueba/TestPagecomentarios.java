...
public class TestPagecomentarios {
    ...
    public void setUp() { //metodo que crea los condiciones iniciales
        ...
        driver.get("http://automationpractice.com/index.php");//envia la url
    ...
    @Test //anotaciones
    public void registro() throws InterruptedException {//metodo donde van las instrucciones
        ...
        if (driver.findElement(registerPage2Locator).isDisplayed()) {
	        ...
	        Select sel=new Select(listState);// con este elemento se seleccion vamos a buscar seleccionar algo de la lista
	          //XXXXXXXXXperform an action on the drop down using select class method XXXXXXXOOOOJJJOOOOO
	        sel.selectByVisibleText("Alabama");//el elemento seleccion busca lo que coincida
	          //Thread.sleep(2000); 
	        driver.manage().timeouts().implicitlyWait(6, TimeUnit.SECONDS);
